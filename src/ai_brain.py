...
def main():
    ...
    base_price = 2850  # RELIANCE-like price
    returns = np.random.normal(0.0002, 0.01, 50)  # Small random returns
    prices = base_price * (1 + returns).cumprod()
    
    market_data = pd.DataFrame({
        'timestamp': dates,
        'open': prices * (1 + np.random.uniform(-0.001, 0.001, 50)),
        'high': prices * (1 + np.random.uniform(0, 0.005, 50)),
        'low': prices * (1 - np.random.uniform(0, 0.005, 50)),
        'close': prices,
